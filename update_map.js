...
// This regex finds the worldLocations array up to the end of the object.
const regex = /worldLocations:\s*\[[\s\S]*?\]\n\};/;
if (regex.test(dataContent)) {
    dataContent = dataContent.replace(regex, newLocations + "\n};");
    fs.writeFileSync(dataPath, dataContent, 'utf8');
    console.log("Successfully updated worldLocations in data.js");
} else {
    // If it doesn't match the specific closing, just replace the array
    const altRegex = /worldLocations:\s*\[[\s\S]*?\]\s*(,\s*|\}\s*;?)/;
    if (altRegex.test(dataContent)) {
       dataContent = dataContent.replace(altRegex, newLocations + "$1");
       fs.writeFileSync(dataPath, dataContent, 'utf8');
       console.log("Successfully updated worldLocations in data.js (fallback)");
    } else {
       console.log("Could not find worldLocations array to replace using regex.");
    }
}
